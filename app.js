...
fetch("data.json")
  ...
  .then(data => {
    ...
    function createNewCommentForm() {
      ...
      const commentInput = newCommentElement.querySelector('textarea');
      const submitButton = newCommentElement.querySelector('button');
    ...
    function createReplyHandler(event) {
      event.preventDefault();
      ...
      const commentElement = replyBtn.closest('.comment-card');
      if (!commentElement) {
        return;
      }
      
      const newCommentElement = createNewCommentForm();
      const replyForm = newCommentElement.querySelector('#new-comment');
      const submitButton = newCommentElement.querySelector('reply-btn');
      
      replyForm.addEventListener('input', () => {
        if (replyForm.value.trim().length > 0) {
      ...
      const handleSubmit = () => {
        const newCommentText = replyForm.value.trim();
        if (newCommentText.length > 0) {
          ...
          repliesSection.appendChild(newCommentCard);
    ...
    replyButtons.forEach(replyBtn => {
      replyBtn.addEventListener('click', createReplyHandler);
    });
